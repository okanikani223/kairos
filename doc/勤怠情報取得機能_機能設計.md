...
```json
{
  ...
  "error": {
    ...
    "message": "位置情報データが不足しているため、勤怠情報を自動作成できません"
  }
}
```
...
function determineWorkingHours(locationData, companyLocation) {
  ...
  // 3. 30分以上滞在している時間帯のみを抽出
  const significantPeriods = filterSignificantPeriods(proximityPeriods, 30);
  
  // 4. 規定時間との整合性を確認し、勤務開始・終了時刻を決定
  const workingHours = alignWithRegulationHours(significantPeriods, companyRegulationHours);
