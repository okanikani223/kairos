...
- 年月指定がある場合で勤怠情報が存在しない場合は、自動勤怠情報作成処理を実行

### 3.2 自動勤怠情報作成
- 指定された年月の位置情報データを時系列で取得
- 位置情報とユーザーの登録された勤務地の位置情報を照合
- 勤務地の位置情報への接近・離脱パターンから勤務開始・終了時刻を判定
- ユーザー設定の規定勤怠時間も参考に勤務時間を算出
- 以下の条件で判定を行う：
  1. 勤務地の近接判定（閾値: 100m以内）
- 算出された勤怠情報を返却（created=true）
...
### 4.2 自動作成に関する要件
- 位置情報が不足している場合は、ユーザー設定の規定勤怠開始・終了時刻からデフォルトの値を生成する
- 自動作成された勤怠情報は「自動作成フラグ」を付与し、手動での確認・修正を促す
- 年月の指定がない場合は自動作成は行わず、既存データのみを返却する
